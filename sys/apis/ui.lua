...
function UI:configure(appName, ...)
	...
	if defaults.device.name then
		...
		if not dev then
			error('Aparato de pantalla invalido')
		end
...
function UI:getPage(pageName)
	...
	if not page then
		error('UI:getPage: pagina invalida: ' .. tostring(pageName), 2)
	end
...
function UI:setPage(pageOrName, ...)
	...
	if type(pageOrName) == 'string' then
		page = self.pages[pageOrName] or error('Pagina invalida: ' .. pageOrName)
	end
...
Agrega elemento (s) a una ventana. Ejemplo:
page:add({
	text = UI.Text {
	  x=5,value='help'
	}
...
Vuelve a dibujar la ventana en el búfer interno.]]
...
Conseguir la documentación para un método.]]
...
Invoca una actualizacion de la pantalla. Automaticamente llamado en el nivel superior después de un evento de entrada.
Llamar para forcar una actualizacion de la pantalla.]]
...
Borra la ventana usando los valores pasados o los predeterminados para esa ventana.]]
...
Escribe texto al camvas.
Si no se especifican colores, Se usaran los colores de la clase Base.
Si la clase Base no tiene colores definidos, los colores se sacaran del contenedor padre.]]
...
Si la función esta presente en una clase, se indica
que este elemento puede aceptar el enfoque. Llamado al recibir el enfoque.]]

UI.Window.docs.setFocus = [[setFocus(ELEMENT el)
Establecer el foco de la página en el elemento pasado.]]
...
Restringe la entrada al árbol del elemento pasado.]]
...
Devuelve una lista de elementos secundarios que pueden aceptar el foco..]]
...
Envía un evento al elemento. Se llama al controlador de eventos del elemento.
Si el controlador de eventos devuelve verdadero, no se realiza ningún procesamiento adicional.
Si el controlador de eventos no devuelve verdadero, entonces el evento se envía al elemento padre
y continúa subiendo por el árbol de elementos.
Si se define un acelerador, el evento acelerado se procesa de la misma manera.
Los aceleradores son útiles para hacer que los eventos sean únicos.]]
...
function UI.Device:addTransition(effect, args, canvas)
	...
	if type(effect) == 'string' then
		effect = Transition[effect] or error('Transicion invalida')
	end
...
local function loadComponents()
	local function load(name)
		...
		if UI[name]._preload then
			error('Error cargando la Interfaz.' .. name)
		end
